...
def _find_locally_or_download_impl(repository_ctx):
  ...
  repository_ctx.template(
      "BUILD", Label("//appengine:{}/sdk.BUILD".format(lang.lower())))
...
find_locally_or_download = repository_rule(
    attrs = {
        ...
        "strip_prefix_pattern": attr.string(
            ...
            mandatory = True,
            doc = "When the zip file is extracted, remove this prefix from all paths. If it includes '{version}', it will be replaced with the version.",
        ),
        "filename_pattern": attr.string(
            default = "google_appengine_{version}.zip",
            mandatory = True,
            doc = "The filename of the SDK zip file to download. If it includes '{version}', it will be replaced with the version.",
        ),
    },
    local = False,
